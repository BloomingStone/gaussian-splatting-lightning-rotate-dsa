...
class RotateXrayDensityControllerImpl(DensityControllerImpl):
    def __init__(self, config, *args, **kwargs) -> None:
        ...
        self.controller_background = controller_config_coronary.instantiate()
    ...
    def before_backward(
        ...
        self.controller_coronary.before_backward(new_outpus, batch, gaussian_model, optimizers, global_step, pl_module)
        
        gaussian_model.state = XrayGassianState.BACKGROUND
        new_outpus = {s: x[gaussian_model.state] for s, x in outputs.items()}
        self.controller_background.before_backward(new_outpus, batch, gaussian_model, optimizers, global_step, pl_module)
