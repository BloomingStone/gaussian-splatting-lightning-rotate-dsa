...
def _skeleton_coords(
    ...
    Args:
        mask: (D, H, W) bool array.
        spacing: Voxel spacing ``(sz, sy, sx)``.

    Returns:
        (N, 3) float64 array of skeleton point coordinates in mm,
        or ``None`` if the skeleton is empty.
    """
...
# ---------------------------------------------------------------------------
# SegmentationMetricsComputer (cached class interface)
# ---------------------------------------------------------------------------
...
class SegmentationMetricsComputer:
    ...
    Usage::
        ...
        computer = SegmentationMetricsComputer(gt_np, aabb_roi_np, spacing)
        for pred_mask in [...]:
            seg_metrics = computer.compute(pred_mask)

        density_metrics = computer.compute_density(density_vol)
    """
    ...
    def __init__(
        ...
        # ---- Pre-compute GT skeleton + KD-tree (one-off) ----
    ...
    def compute(self, pred: torch.Tensor) -> dict[str, float]:
        ...
        # --- MONAI expects (1, 1, D, H, W) float ---
        pred_bc = pred.unsqueeze(0).unsqueeze(0).float()
        gt_t = torch.from_numpy(self._gt_np).to(
        ...
        if one_empty:
            ...
            hd95 = float(
                compute_hausdorff_distance(
                    pred_bc, gt_bc, percentile=95, spacing=self._spacing,
                ).item()
            )
        ...
        # ── Centerline distances (CPU, using cached GT skeleton) ──
        if one_empty:
    ...
    def compute_density(self, density: torch.Tensor) -> dict[str, float]:
        ...
        """
        roi = torch.from_numpy(self._aabb_roi_np).to(
            device=density.device, dtype=torch.bool,
        )
        gt_t = torch.from_numpy(self._gt_np).to(
            device=density.device, dtype=torch.bool,
        )
