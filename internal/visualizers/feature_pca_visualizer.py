...
def pca_colormap(image, ignore_zeros: bool = True):
    ...
    if ignore_zeros:
        ...
        valids = torch.ones(image.shape[0], dtype=torch.bool, device=image.device)
...
class FeaturePCAVisualizer(Visualizer):
    def __init__(
        self, 
    ...
    def process(self, image: Tensor) -> Tensor:
        return pca_colormap(image, ignore_zeros=self.ignore_zeros)