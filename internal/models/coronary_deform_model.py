...
from internal.utils.rigid_utils import exp_se3, from_homogenous, to_homogenous
...
class DeformModel(nn.Module):
    ...
    def forward(
        ...
        d_rotation = F.normalize(self.gaussian_rotation(h), dim=-1)
        d_scaling = self.gaussian_scaling(h)
        
        xyz = xyz + d_xyz
        rotation = GaussianTransformUtils.quat_multiply(rotation, d_rotation)
        scale = scale + d_scaling
        return xyz, rotation, scale
...
# TODO 现在有问题，在test中forward正常，但是在训练过程的validation中中渲染图片错误，后续无法继续训练
class DeformModel6DoF(DeformModel):
    def __init__(
            self,
            network_factory: NetworkFactory,
            cfg: DeformModelConfig = DeformModelConfig(),
    ):
        super().__init__(network_factory, cfg)
    
    @override
    def _build_deform_linears(self):
        W = self.cfg.W
        _linear = self.network_factory.get_linear
        self.branch_w = _linear(W, 3)
        self.branch_v = _linear(W, 3)
        self.gaussian_rotation = _linear(W, 4)
        self.gaussian_scaling = _linear(W, 3)
    ...
    def forward(
        ...
        w = self.branch_w(h).to(xyz.dtype)
        v = self.branch_v(h).to(xyz.dtype)
        theta = torch.norm(w, dim=-1, keepdim=True)
        w = w / (theta + 1e-8)

        # exp(se3)
        screw_axis = torch.cat([w, v], dim=-1)
        SE3 = exp_se3(screw_axis, theta)
        xyz = from_homogenous(torch.bmm(SE3, to_homogenous(xyz).unsqueeze(-1)).squeeze(-1))

        d_rotation = F.normalize(self.gaussian_rotation(h), dim=-1).to(xyz.dtype)
        rotation = GaussianTransformUtils.quat_multiply(rotation, d_rotation)
        
        d_scaling = self.gaussian_scaling(h).to(xyz.dtype)
        scale = scale + d_scaling
